    ...
    $.ajax({
      ...
      success: function (dados) {
        if (dados != "") {
          ...
          $(`<option value="${resPac[0].idPaciente}">${ruaPaciente}</option>`).appendTo('select[name="ruaPaciente"]');

          $(`<option value="${resPac[0].idPaciente}">${bairroPaciente}</option>`).appendTo(
            'select[name="bairroPaciente"]'
          );
        }
